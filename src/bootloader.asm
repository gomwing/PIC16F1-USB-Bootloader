...
; Copyright (c) 2021, Sasapea's Lab. (https://lab.sasapea.mydns.jp)
; derived from
; USB 512-Word CDC Bootloader for PIC16(L)F1454/5/9
...
SERIAL_NUMBER_DIGIT_CNT	equ	4
	ifndef SERIAL_NUMBER_DIGIT
SERIAL_NUMBER_DIGIT equ 0x0001	; Why doesnt 'equ' work here? Go figure
	endif
...
INTERRUPT_VECT    equ 0x0004
DEVICE_DESCRIPTOR equ BOOTLOADER_SIZE-ALL_DESCS_TOTAL_LEN
...
; Print any pending characters in the log
  ...
  banksel	PIR2
	btfss PIR2,USBIF
	goto	_loop
  call ret  ; simulate interrupt call overhead
  call _bootloader_interrupt
  banksel	PIR2
	bcf	PIR2,USBIF
	goto	_loop
...
; and the requested length
	tstf	BANKED_EP0OUT_BUF+wLengthH  	; just ignore high byte...
	bnz  _usb_ctrl_complete		; if W <= f, no need to adjust
...
ep0_read_in
	...
	movwf	FSR0L
	movlw	high DEVICE_DESCRIPTOR|0x80
	movwf	FSR0H
...
; $add$ do not run application if the watchdog timed out (providing a mechanism for the app to trigger a firmware update)
  if WDT_BOOTLOADER_MODE  ; $add$
  btfss	STATUS,NOT_TO     ; $add$
	goto	_bootloader_main  ; $add$
  endif                   ; $add$

; $add$ Check for Bootloader Mode (Software Reset with FSR1L=0xDF and FSR0L=0xBE)
  movfw FSR1L            ; $add$
  sublw 0xDF             ; $add$
  bnz   _check_app_entry ; $add$
  movfw FSR0L            ; $add$
  sublw 0xBE             ; $add$
  bz   _bootloader_main  ; $add$
_check_app_entry         ; $add$
...
;	return				; Z flag will be unset if app code is present
  skpz         ; $add$
	return				; $add$ Z flag will be unset if app code is present
	moviw	FSR0++  ; $add$
  iorlw 0xC0   ; $add$
	incw				  ; $add$ if W was 0xFF, it'll be 0 now
	return				; $add$ Z flag will be unset if app code is present
...
;CALL_MANAGEMENT_FUNCTIONAL_DESCRIPTOR
	...
	dt	0x01		; bmCapabilities (doesn't handle call management)
...
SN1	equ	(SERIAL_NUMBER_DIGIT>>12) & 0xF
SN2	equ	(SERIAL_NUMBER_DIGIT>>8) & 0xF
SN3	equ	(SERIAL_NUMBER_DIGIT>>4) & 0xF
SN4	equ	SERIAL_NUMBER_DIGIT & 0xF
